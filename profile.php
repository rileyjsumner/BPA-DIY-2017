...
<html>
    ...
    <body>
        ...
        <div class='row'>
            <div class='col-1'>
                <?php 
                    $sqlAdmin = "SELECT * FROM `posts` WHERE `status`='pending';";
                    if(Token::check(Verify::get('token')) && $profile == "dogwood_staff") {
                        $resultAdmin = mysqli_query($conn, $sqlAdmin);
                        while($rowA = mysqli_fetch_assoc($resultAdmin)) {
                            $postsA[$rowA["timestamp"]] = array("title"=>$rowA["title"], 
                                                            "user"=>$rowA["user"], 
                                                            "description"=>$rowA["description"], 
                                                            "steps"=>$rowA["steps"],
                                                            "materials"=>$rowA["materials"],
                                                            "tips"=>$rowA["tips"],
                                                            "time"=>$rowA["estTime"],
                                                            "cost"=>$rowA["estCost"],
                                                            "reviews"=>$rowA["ratings"],
                                                            "rating"=>$rowA["rated"],
                                                            "tags"=>$rowA["tags"],
                                                            "postID"=>$rowA["postID"],
                                                            "stamp"=>$rowA["timestamp"]);

                    }
                }   
                ksort($postsA);
                foreach(array_reverse($postsA) as $elemA) { 
                    $photoSQLA = "SELECT * FROM `upload` WHERE `postID`=" . $elemA["postID"] . ";";
                    $resultPhotoA = mysqli_query($conn, $photoSQLA);
                    $photoURLA = "";
                    if (mysqli_num_rows($resultPhotoA) > 0) {
                        while ($row4 = mysqli_fetch_assoc($resultPhotoA)) {
                            $photoURLA = $row4["imgLoc"];
                        }
                    }

                ?>
                <div class='project'>
                    <h2><?php echo $elemA["title"]; ?></h2>
                    <p><?php echo $elemA["description"]; ?></p>
                    <p>Rating: <?php if($elemA["rating"] !== null){ echo $elemA["rating"], " stars"; } else { echo "This project has not yet been rated"; } ?></p>
                    <p><?php echo "<a href=profile.php?User=".$elemA["user"].">".$elemA["user"]."</a>"; ?></p>
                    <?php if($photoURLA !== ""){ ?>
                    <img src="uploads/<?php echo $photoURLA; ?>"/>
                    <?php } ?>
                    <p>Estimated Time: <?php echo $elemA["time"], " minutes"; ?></p>
                    <p>Estimated Cost: <?php echo "$", $elemA["cost"]; ?></p>
                    ...
                    <div class='hidden' id='details<?php echo $x; ?>'>
                        <p>Materials List:</p>
                        <?php 
                            $stepsA = explode("~", $elemA["steps"]);
                            $itemsA = explode("~", $elemA["materials"]);
                            echo '<ul>';
                            for($z = 1; $z < sizeof($itemsA); $z++) {
                                $contentsA = explode(",", $itemsA[$z]);
                                echo "<li><a href='".$contents[0]."' target='_blank'>".$contentsA[1]."</a></li>";
                            }
                            echo '</ul>';
                        ?>
                        <p>Procedure:</p>
                        <?php 
                            ...
                            echo '<ol>';
                            for($y = 1; $y < sizeof($stepsA); $y++) {
                                echo '<li>'.$stepsA[$y].'</li>';
                            }
                            echo '</ol>';
                        ?>
                        <p>Tips: <?php echo $elemA["tips"]; ?></p>

                        <p><?php echo date("F j, Y @ g:i a", strtotime($elemA["stamp"])); ?></p>

                    </div>
                    <button type='submit' name='accept' onclick='accept(<?php echo $elemA["postID"]; ?>)'>Accept</button>
                    <button type='submit' name='decline' onclick='decline(<?php echo $elemA["postID"]; ?>)'>Decline</button>
                </div>
                    <?php
                        }
                ?>
            ...
            <div class='col-3'>
             <?php   
                $sql2 = "SELECT * FROM `posts` WHERE `user` = '$profile' AND `status`='confirmed';";
